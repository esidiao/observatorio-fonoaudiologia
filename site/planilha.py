...
Uso (normalmente chamado por site/build.py):
    python site/planilha.py --saida site/dist/dados/observatorio-fonoaudiologia.xlsx
...
CABECALHO = Font(name="Calibri", size=11, bold=True, color="FFF2F7F5")
FUNDO_CABECALHO = PatternFill("solid", fgColor=VERDE)
TITULO = Font(name="Calibri", size=13, bold=True, color=VERDE)
NOTA = Font(name="Calibri", size=9, italic=True, color="FF6B7280")
BORDA = Border(bottom=Side(style="thin", color="FFE7E3DC"))
...
def _escrever_cabecalho(aba, colunas, larguras=None):
    ...
    # Painel congelado: sem isso, rolar 629 municípios deixa o leitor sem saber
...
def aba_municipios(wb, dir_municipios, nomes_uf):
    ...
    campos = ["vagas_presencial", "cursos_presencial", "n_ies", "matriculas",
              ...
              "fonoaudiologos_sus", "estabelecimentos_servico_fono"]
...
def aba_leia_me(wb, nacional, proveniencia):
    ...
    linhas = [
        ("Observatório Nacional da Formação em Fonoaudiologia", None),
        ("", None),
        ("Autoria", "Edson Sidião de Souza Júnior — sidiao@i9educar.com"),
        ("Site", "https://esidiao.github.io/observatorio-fonoaudiologia/"),
    ...
    linhas += [
        ...
        ("DIREITOS", None),
        ("Dados primários",
         "Públicos, do INEP, do Ministério da Saúde e do IBGE."),
    ...
    for i, (a, b) in enumerate(linhas, start=1):
        ...
        if a and a.isupper() and b is None:
            ca.font = Font(bold=True, color=TEAL[2:], size=11)
        elif i == 1:
            ca.font = TITULO
        else:
...
def gerar(saida, dir_dados=None, nomes_uf=None):
    ...
    wb.properties.title = "Observatório Nacional da Formação em Fonoaudiologia"
    ...
    wb.properties.description = (
        ...
        "dos cursos de Fonoaudiologia no Brasil.")
...
def main():
    ...
    p.add_argument("--saida", default=str(
        RAIZ / "site" / "dist" / "dados" / "observatorio-fonoaudiologia.xlsx"))
