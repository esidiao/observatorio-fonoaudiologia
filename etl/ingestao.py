...
Entrada:  etl/dados/curso_fonoaudiologia_<ano>.csv  (de extrair_censo.py)
...
Medido no Censo 2024 de Fonoaudiologia: 125 linhas presenciais, 793 de polo e
21 de sede. As 21 linhas de sede carregam as 12.876 vagas EaD inteiras; as 793
de polo carregam 8.044 matrículas e nenhuma vaga. Somar `QT_VG_TOTAL` por
`SG_UF` sem separar as camadas devolve as vagas EaD como zero — e o número
resultante parece plausível, que é o que torna o erro caro.
...
def carregar_cursos(ano):
    caminho = DADOS / f"curso_fonoaudiologia_{ano}.csv"
    if not caminho.exists():
        raise SystemExit(f"[ERRO] {caminho} ausente. Rode etl/extrair_censo.py antes.")
    with open(caminho, encoding="utf-8") as f:
