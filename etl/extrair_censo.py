...
Uso:
    ...
    python etl/extrair_censo.py --ano 2024 --rotulo "Fonoaudiologia"
    python etl/extrair_censo.py --ano 2024 --listar-rotulos fono
...
acento, sem caixa, sem espaço nas bordas) — jamais `in`. Para Fonoaudiologia a
diferença é concreta: existe `Produção fonográfica`, que contém "fono". Para
Medicina seria pior ainda, capturando `Biomedicina` e `Medicina veterinária`.
...
def extrair(ano, rotulo):
    ...
    # 4 letras, não 5: com 5 o fragmento de "Fonoaudiologia" vira "FONOA", que
    # não casa com "PRODUCAO FONOGRAFICA" — justamente o vizinho que o relatório
    # existe para expor. O aviso que não dispara no caso conhecido não vale nada.
...
def main():
    ...
    p.add_argument("--rotulo", default="Fonoaudiologia",
                   help="rótulo CINE EXATO (default: Fonoaudiologia)")
